...
unif.seed _ 48298; int.seed _ 48248;
unif.seed1 _ unif.seed; unif.seed2 _ unif.seed; unif.seed3 _ unif.seed;
int.seed1 _ int.seed; int.seed2 _ int.seed; int.seed3 _ int.seed;
nvars _ 7;
ncomponents _ 1;
...
#load up new data
#us _ read.csv(file="../../../data/final/us.with2000censusAKRIDEHI.26july.csv",header=T);

us _ read.csv(file="/mnt/raid/xchg/election/data/final/us.with2000censusAKRIDEHI.26july.csv",header=T);

us$state.orig _ us$state;
us$state _ as.vector(us$state);
indx _ us$state=="CT" | us$state=="DE" | us$state=="HI" | us$state=="RI";
us$state[indx] _ rep("A1",sum(indx));

tmp <- as.factor(us$state)
states <- levels(tmp)
states <- states[ states != "DC" & states != "MI"];
...
parms.use       _ as.data.frame(states);
names(parms.use) _ "state";
parms.use$run   _ rep("begin",nstates);
parms.use$fit   _ rep(8888,nstates);
parms.use$x1    _ rep(0,nstates);
parms.use$x2    _ rep(0,nstates);
parms.use$x3    _ rep(0,nstates);
parms.use$pr1   _ rep(0,nstates);
parms.use$int1  _ rep(0,nstates);
parms.use$int2  _ rep(0,nstates);
parms.use$int3  _ rep(0,nstates);

#parms for A1
parms.use[1,3:10] _ c(9999,
                      -7.362488e+00, 2.164819e+00, 1.472589e+01, -7.451106e-02,
                      -4.925120e-01, -6.954997e-01, -1.867386e-01);

print(parms.use);

parm.mat _ as.matrix(cbind(parms.use$x1,parms.use$x2, parms.use$x3, parms.use$pr1,
                           parms.use$int1, parms.use$int2,parms.use$int3));
...
for (statename in states.order) 
  {
    ...
    nintercepts _ 3;
    int1        _ as.vector(us.tmp$state.orig=="CT");
    int2        _ as.vector(us.tmp$state.orig=="DE");
    int3        _ as.vector(us.tmp$state.orig=="HI");    
    us.tmp$int1 _ int1;
    us.tmp$int2 _ int2;
    us.tmp$int3 _ int3;    
...
############################################    
    ...
    fit.lqd2 _ function(foo)
      {
#        return(.Call("fit_lqd2",as.integer(nparms), as.integer(ncomponents), as.integer(obs),
...
#                     as.integer(nintercepts), as.vector(int1), as.vector(int2)
#                     ));

        mu _ foo[1] + foo[2]*prep96 + foo[3]*pperot96 + foo[4]*pr1 +
          foo[5]*int1 + foo[6]*int2 + foo[7]*int3;        
        y _ 1/(1+exp(-1*mu));
        
        rawres _ ( pop*(pbuchanan-y) )/( sqrt(pop*y*(1-y)) );
        dif _ abs(outer(rawres,rawres,"-")[outer(1:obs,1:obs,">")])
    ...
    fit.lqd2.reload _ function()
      {
#        if (is.loaded("fit.lqd2"))
#          {
#            dyn.unload("fit_lqd2.so");
#          }
#        dyn.load("fit_lqd2.so");
      }
...
#      {
        glm0 _ glm(pbuchanan ~ prep96 + pperot96 + int1 + int2 + int3,
                   family=binomial(link="logit"), weights=vtot00,
                   data=us.tmp)
        cat("GLM0:\n");
        print(summary(glm0));
        
        glm1 _ glm(pbuchanan ~ prep96 + pperot96 + pr1 + int1 + int2 + int3,
                   family=binomial(link="logit"), weights=vtot00,
                   data=us.tmp)
        cat("GLM1:\n");
        print(summary(glm1));
    ...
    if (!SKIP)
      {
        ...
        if (parms.use$fit[tindx]!=88888)
          {
            starting.values _
              c(parms.use$x1[tindx],parms.use$x2[tindx],parms.use$x3[tindx],
                parms.use$pr1[tindx],parms.use$int1[tindx],parms.use$int2[tindx],
                parms.use$int3[tindx]);
          } else {
        ...
        for (id in 1:nparms) {
          mdomains[id,1] _
            starting.values[id] - domain.scale*max(abs(starting.values[id]),min.domain.width);
          mdomains[id,2] _
            starting.values[id] + domain.scale*max(abs(starting.values[id]),min.domain.width);
        }
        fit.lqd2.reload();
        gen1 _ genoud(fit.lqd2,nvars=nvars,max=F,starting.values=starting.values,
                      Domains=mdomains,pop.size=pop.size,
                      wait.generations=wait.generations,
                      max.generations=max.generations,                  
                      hard.generation.limit=T,
                      BFGS=T, gradient.check=gradient.check,
                      project.path=project,
                      unif.seed=unif.seed1,int.seed=int.seed1);
        ...
        wg.boost _ 3;
        fit.lqd2.reload();
        gen2 _ genoud(fit.lqd2,nvars=nvars,max=F,starting.values=gen1$par,
                      ...
                      gradient.check=F,
                      project.path=project,
                      unif.seed=unif.seed2,int.seed=int.seed2);
        ...
        wg.boost _ 5;
        fit.lqd2.reload();
        gen3 _ genoud(fit.lqd2,nvars=nvars,max=F,starting.values=gen2$par,
                      Domains=mdomains,pop.size=pop.boost*pop.size,
                      wait.generations=wg.boost*wait.generations,
                      max.generations=wg.boost*wait.generations*10,
                      hard.generation.limit=T,                  
                      ...
                      project.path=project,
                      unif.seed=unif.seed3,int.seed=int.seed3);
            ...
            if (genoud.best$value < parms.use$fit[j])
              {
                ...
                parms.use$int1[j] _ genoud.best$par[5];
                parms.use$int2[j] _ genoud.best$par[6];
                parms.use$int3[j] _ genoud.best$par[7];                
    ...
    residual.generator _ function (foo, x)
      {
        ...
        mu _ foo[1] + foo[2]*tprep96 + foo[3]*tpperot96 + foo[4]*tpr1 +
          foo[5]*tint1 + foo[6]*tint2 + foo[7]*tint3;
...
# tanh estimator
    ...
    rtanh <-
      rewtanh(parm.mat[j,], s0^2, residuals$res,
              x[,4], x[,3]*x[,4], cbind(1,prep96,pperot96,pr1,int1,int2,int3));
    ...
    fn.county.results _ function (foo, x, sigma, Hdiag)
      {
        mu _ foo[1] + foo[2]*x$prep96 + foo[3]*x$pperot96 +
          foo[4]*x$pr1+foo[5]*x$int1 + foo[6]*x$int2 + foo[7]*x$int3;
